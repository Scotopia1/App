...
public class LoginController {
    ...
    public void initialize() {
        ...
        //make the login button click when the enter key is pressed
        rootPane.addEventHandler(KeyEvent.KEY_PRESSED, (key) -> {
            if (key.getCode() == KeyCode.ENTER) {
                LoginButton.fire();
            } else if (key.getCode() == KeyCode.F11) {
                FullScreen.toggleFullScreen();
                System.out.println("Full Screen Toggled " + Date.from(new Date().toInstant()));
            }
        });
    }
